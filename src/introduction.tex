...
Use a field survey to orient the reader~\cite{survey}, and a source on the
application to support the motivation~\cite{application}. These citations,
like all references in this template, are fictional examples of citation
roles and must be replaced with verified publications.
...
Anchor the comparison in a foundational approach~\cite{foundational} and
a relevant alternative~\cite{alternative}. Cite the work at the point where
you discuss its scope; a citation alone does not establish a limitation.
...
\figref{fig:hook} highlights the central idea or motivating example.
\figref{fig:overview} provides a full-width overview of the method.
...
The remainder of this paper reviews related work in \secref{sec:related},
defines the problem in \secref{sec:problem}, describes the method in
\secref{sec:method}, and presents evaluation in \secref{sec:experiments}.
\secref{sec:discussion} discusses limitations.
